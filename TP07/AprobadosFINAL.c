...
char ** aprobados(TipoNombre alumnos, int notas[]) {
	...
	for(i = 0; strcmp(alumnos[i], EOA); i++) {
		if(notas[i] >= 4) {
			printf("A\n");
			...
			aprobados[aprobaron] = malloc(TOPE * sizeof(**aprobados));
			if(aprobados[aprobaron] == NULL)
				return ERROR;
			strcpy(aprobados[aprobaron], alumnos[i]);
			aprobaron++;
		}
	}
